...
export async function GET() {
  ...
  try {
    ...
    console.error("RPC Fetch failed, using high-fidelity fallbacks");
  }
  
  // High-quality static arenas for the hackathon MVP
  const fallbackMarkets = [
    { id: "colosseum-winner", title: "WHICH ELITE ENTITY SECURES THE GRAND PRIZE?", volume: "42,000 SOL", category: "META", type: "multiple" },
    { id: "sol-tps-feb", title: "Solana handles > 50,000 TPS average in Feb?", yesOdds: 0.65, noOdds: 0.35, volume: "14,200 SOL", category: "NETWORK" },
    ...
    { id: "firedancer-mainnet", title: "Firedancer is live on Mainnet-Beta by Q2?", yesOdds: 0.40, noOdds: 0.60, volume: "12,000 SOL", category: "NETWORK" }
  ];

  // Merge: if RPC has data, show it first, then fill rest with interesting markets
  const finalMarkets = markets.length > 0 ? markets : fallbackMarkets;
